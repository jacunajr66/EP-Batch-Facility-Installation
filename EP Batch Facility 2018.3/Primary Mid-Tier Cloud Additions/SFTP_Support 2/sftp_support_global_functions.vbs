...
Function ProcessControlFile (controlFile, sqlLoadType)
...
' =============================================================================	
					...
					tableName =  Right(Left(dataFile,dataFileLength-4),dataFileLength-19)
					...
					if Err.Number <> 0 then
						...
						'
						' Kludge because SQLLDR files do not have a label to trigger on when changing the value so we have to look
						' for both possible values of APPEND and TRUNCATE then replace with the specified value.
						'
						if sqlLoadType <> "" then
							regEx.pattern = "APPEND .*"
							strText = regEx.replace(strText, sqlLoadType)
							regEx.pattern = "TRUNCATE .*"
							strText = regEx.replace(strText, sqlLoadType)
						end if
