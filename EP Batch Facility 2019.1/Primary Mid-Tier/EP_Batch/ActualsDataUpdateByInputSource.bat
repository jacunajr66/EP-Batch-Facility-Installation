...
IF "%2" == "" (
   ECHO Usage: [FTF] [inputsource] [trusted/untrusted]
...
RowCount %FOUNDATIONUSER% %DATASOURCE% %2
...
IF %ERRORLEVEL% EQU 9000 (
   ...
   pkbfdatupd / %1 inputsource=%2 continueiferrors=%UNTRUSTED% bulkloadgranite=%BULKLOAD% dropindexgranite=y

   RowCount %FOUNDATIONUSER% %DATASOURCE% PF_DU_%1_ERR %ALLOWED_ERROR_CODES%

   IF NOT ERRORLEVEL 1 (
      pkbfdatrm / type=inputsources inputsource=%2 ftf=%1 analyze=n empty=%EMPTY%
   ) ELSE (
      SET ERRORLEVEL=9999
   )
...
IF %ERRORLEVEL% EQU 9001 (
   pkbfdatupd / %1 inputsource=%2 continueiferrors=%UNTRUSTED% bulkloadgranite=%BULKLOAD% dropindexgranite=y restart=y
