...
SET EXPIREDAYS=28
SET FILEPREFIX=EXT_EPSD_IMPORT_MEMBER_REL*

SET ARCHIVEDIR=.\Archive
SET BADDIR=.\Bad\
SET DISCARDDIR=.\Discard\
SET LOGDIR=.\Log\
SET STRUCTUREDIR=.\Archive\MemberRelationships.%DATE:~-2,2%-%DATE:~-10,2%-%DATE:~-7,2%
...
FOR %%a IN (%FILEPREFIX%) DO (
   ...
   ECHO ^(inputsequence,actioncode,bus_entity_tech_key,child_member_id,child_level_def_id,parent_member_id,parent_level_def_id,messageid,messagetext^)>>%CONTROLFILE%
